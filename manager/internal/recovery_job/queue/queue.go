...
// TODO: Pause 기능을 사용하지 않음. 주석처리
//// SetJobStatePaused 작업을 일시중지한다.
//func SetJobStatePaused(txn store.Txn, job *migrator.RecoveryJob, pausingTime int64) error {
//	logger.Infof("[queue-SetJobStatePaused] Start: job(%d)", job.RecoveryJobID)
//
//	status, err := job.GetStatus()
//	if err != nil {
//		logger.Errorf("[queue-SetJobStatePaused] Could not get job status: dr.recovery.job/%d/status. Cause: %+v", job.RecoveryJobID, err)
//		return err
//	}
//
//	status.StateCode = constant.RecoveryJobStateCodePaused
//	status.PausedAt = time.Now().Unix()
//	status.ResumeAt = time.Now().Unix() + pausingTime
//
//	if err = job.SetStatus(txn, status); err != nil {
//		logger.Errorf("[queue-SetJobStatePaused] Could not set status: dr.recovery.job/%d/status {%s}. Cause: %+v",
//			job.RecoveryJobID, status.StateCode, err)
//		return err
//	}
//
//	logger.Infof("[queue-SetJobStatePaused] Success - set status: dr.recovery.job/%d/status {%s}",
//		job.RecoveryJobID, status.StateCode)
//
//	if err = internal.PublishMessage(constant.QueueRecoveryJobMonitor, migrator.RecoveryJobMessage{
//		JobID:  job.RecoveryJobID,
//		Status: status,
//	}); err != nil {
//		logger.Warnf("[queue-SetJobStatePaused] Could not publish job(%d) status. Cause: %+v", job.RecoveryJobID, err)
//	}
//
//	return nil
//}
//
//// SetJobStateRunningByResume 일시중지된 작업을 재개한다.
//func SetJobStateRunningByResume(txn store.Txn, job *migrator.RecoveryJob) error {
//	logger.Infof("[queue-SetJobStateRunningByResume] Start: job(%d)", job.RecoveryJobID)
//
//	status, err := job.GetStatus()
//	if err != nil {
//		logger.Errorf("[queue-SetJobStateRunningByResume] Could not get job status: dr.recovery.job/%d/status. Cause: %+v", job.RecoveryJobID, err)
//		return err
//	}
//
//	status.StateCode = constant.RecoveryJobStateCodeRunning
...
//
//	if err = job.SetStatus(txn, status); err != nil {
//		logger.Errorf("[queue-SetJobStateRunningByResume] Could not set status: dr.recovery.job/%d/status {%s}. Cause: %+v",
//			job.RecoveryJobID, status.StateCode, err)
//		return err
//	}
//
//	logger.Infof("[queue-SetJobStateRunningByResume] Success - set status: dr.recovery.job/%d/status {%s}",
//		job.RecoveryJobID, status.StateCode)
//
//	if err = internal.PublishMessage(constant.QueueRecoveryJobMonitor, migrator.RecoveryJobMessage{
//		JobID:  job.RecoveryJobID,
//		Status: status,
//	}); err != nil {
//		logger.Warnf("[queue-SetJobStateRunningByResume] Could not publish job(%d) status. Cause: %+v", job.RecoveryJobID, err)
//	}
//
//	return nil
//}
